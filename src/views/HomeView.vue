...
import { Download, Phone, Settings } from 'lucide-vue-next'
...
<template>
  <div class="text-[#f0f0f0] font-inter flex flex-col pt-20 items-center gap-5 mx-10 max-h-screen">
    <div class="space-y-2">
      ...
      <p class="text-3xl text-gray-400 text-center font-medium">Frontend Developer</p>
    </div>
    ...
    <ul class="flex items-center justify-center gap-3 mt-10">
      <li v-for="(buttons, index) in homeButtons" :key="index">
        <a
          ...
          class="bg-white text-black px-4 py-2 rounded-md flex justify-center items-center gap-2 cursor-pointer hover:scale-105 hover:shadow-gray-100 transition-transform duration-300"
        >
          <component :is="buttons.icon" :size="18" /> {{ buttons.label }}
        </a>
        <!-- <button
          v-else-if="buttons.action === 'contact'"
          @click="contactMe"
          class="bg-white text-black px-4 py-2 rounded-md flex justify-center items-center gap-2 cursor-pointer hover:scale-105 hover:shadow-gray-100 transition-transform duration-300"
        >
          <component :is="buttons.icon" :size="18" /> {{ buttons.label }}
    ...
    <div class="mt-10">
      <h2 class="text-3xl flex justify-center items-center gap-2">
        <span><Settings :size="35" /></span>Tools & Technologies
      </h2>
      <Card class="bg-black mt-5">
        <CardContent class="bg-black">
          <div class="flex flex-wrap gap-5 w-110 justify-center items-center text-white">
            <div
              v-for="(stack, index) in techStacks"
              :key="index"
              class="flex flex-col items-center justify-center gap-2 w-20 h-20 hover:bg-gray-800 hover:rounded-2xl hover:font-medium"
            >
              <Icon
                :icon="stack.icon"
                width="30"
              ...
              />
              <span class="flex justify-center items-center text-center font-['Arial'] text-sm">{{
                stack.label
              }}</span>
            </div>
          </div>
